...
moedict-data-twblg)跟目前題庫詞數,滑鼠移過去可以點連結。搜尋框下方有一個
預設收合的「怎麼查?」說明區塊(`<details>`,點了才展開,不占版面),裡面
...
reference/                    使用者提供的參考資料(不是教育部辭典,唯讀查證用)
reference/*.md                 原始參考資料(例如工程車台語詞彙整理)
reference/*.json               上面 .md 的結構化 JSON 版本,給其他工具用(見開發紀錄)
```

## 開發紀錄

- 2026-09-20:使用者問「拔河搶答也可以直接按人偶嗎?」——原本只有
  「搶答(D)/搶答(K)」按鈕可以搶答,現在直接點角色本身也可以,效果跟按鈕
  完全一樣(`GameState.buzz()` 本來就會擋不在搶答階段的呼叫,不需要另外
  判斷)。角色的手腳都只是細線條(沒有填色),直接點線條本身很難點準,所以
  在角色底下疊一個蓋住整個角色範圍的透明圓形當作點擊熱區,手機上也比較
  好按。用 Playwright 測試時發現角色本身有持續晃動的動畫,導致 Playwright
  預設的「等元素穩定下來才點擊」機制永遠等不到穩定、直接逾時——這只是
  測試工具的限制,真實使用者點擊/觸控不會有這個問題,改用 `{ force: true }`
  繞過穩定性檢查來測試,桌面滑鼠點擊跟手機觸控(`devices['Pixel 7']` +
  `.tap()`)都確認搶答動作正確觸發、不會誤觸對方隊伍。
- 2026-09-20:使用者提供小孩畫的一張彩色角色蠟筆畫(圓頭巾狀身體、白色
  臉、黑點眼睛、腮紅、Y 字形棒狀手腳),要求拔河場景的角色照這張畫重畫、
  動作也要更生動。原本的角色是純線條火柴人(`.leg`/`.torso`/`.arm` 直線
  + 一個圓形頭),改成 SVG path 畫的「圓頭巾小怪獸」:身體是蛋形/頭巾形
  的色塊,裡面嵌一個白色圓臉(眼睛、微笑嘴、腮紅),一手伸向繩子(維持
  原本抓繩位置不變,拔河動畫的視覺邏輯不用改),另一手舉高像歡呼,腳底
  加了像樹根一樣分岔的腳趾線條,呼應原圖的畫風。表情會隨遊戲狀態切換:
  平常張眼微笑;答對瞬間(`bigPull`)嘴巴張開像用力、腮紅加深;被拉走的
  那隊(`stumble`)眼睛變成驚訝的 ×;遊戲結束獲勝方(`victory`)跳動+
  張嘴笑;落敗方(`defeated`)閉眼癱倒——這些都是靠 CSS 對既有的
  `bigPull`/`stumble`/`victory`/`defeated` class(`src/ui.js` 本來就有在
  切換,沒有改 JS)另外切換子元素的 `opacity` 做出來的,不用額外寫動畫
  觸發邏輯。動畫本身也調得更誇張(後仰角度、縮放、平常的上下彈動)比較
  有「用力拔河」的感覺。過程中用 Playwright 截圖時發現一個問題:臉的
  底色原本用 `var(--card-bg)`(想說深色模式會自動變色),但深色模式下
  `--card-bg` 太暗,臉會幾乎全黑,深色的眼睛/嘴巴完全看不見——改成固定
  用一個淺色(`#fdfaf3`),不跟著深色模式變,才修好。桌面/手機寬度、
  淺色/深色模式都截圖確認過。
...
- 2026-09-19:使用者問「新增的 reference 可以 output 出來,json 可以給其它
  ...
  把原始 markdown(31 段、72 個詞條,含少數沒有對應漢字的純羅馬字借詞)
  逐段手動轉錄成結構化 JSON,每個分類保留中文說明跟底下的 `{hanzi, poj}`
  詞條清單;純羅馬字沒有漢字的詞 `hanzi` 是 `null`;少數「羅馬字語幹+車」
  的混合寫法(例如「lâm-a-khóng車」)額外帶一個 `raw` 欄位保留原始寫法,
  沒有勉強拆成 hanzi/poj。手動轉錄是刻意的選擇——這份原始檔案格式不太
  規則(全形/半形括號混用、部分項目被 markdown 斷成兩段),寫 regex
  硬解比人工逐行核對更容易出錯。轉錄完用程式核對過詞條數量(31 類、
  72 詞)跟抽樣比對過幾個帶特殊變音符號的詞條(鋏仔車、鋼牙、干樂等)
  確認跟原始檔案一致。另外每個詞條標了 `usedInGame`(布林值,只在真的用
  在遊戲「工程車」模式的 15 個詞上出現),清楚說明這份 JSON 本身**沒有**
  重新查證羅馬字發音,只是原始文字的結構化版本,跟遊戲題庫本身的查證
  流程(部分詞查證過教育部辭典)是兩件事,不要混為一談。這份檔案跟遊戲
  執行期完全無關(不會被 `index.html`/`find.html` 載入),單純是輸出給
  使用者自己或其他工具用,所以沒有跟著 bump `data/version.js`。
- 2026-09-19:使用者問「擴充辭典含 reference 的內容嗎?搜尋的到?」——實際
  用程式查證後回報:工程車/植物模式裡靠 `reference/` 或使用者對話提供的
  詞(堆高機、流籠車、沙石仔車、鴨母車、鐵輪、七里香、福木、芙蓉)完全
  不在教育部辭典裡,`find.html` 查不到;另外「山貓」「豬哥牙」「干樂」
  「田螺」這 4 個詞辭典裡查得到,但顯示的是辭典本身的釋義(雲豹/犬齒/
  陀螺/軟體動物),不是工程車俗稱的意思。使用者確認後,對後面這 4 個詞
  加了處理:`src/find.js` 新增 `VEHICLE_SLANG_NOTE` 對照表,`find-ui.js`
  查到這幾個字的時候,在結果卡片裡多顯示一行橘色提醒,說明這個漢字在
  工程車俗稱裡還有什麼意思,避免使用者誤以為辭典釋義是查詢出錯。前面那
  8 個完全沒被辭典收錄的詞維持查不到的原狀,沒有額外處理——`find.html`
  本來就只查教育部辭典本身收錄的內容,這是正確行為,不是需要修的問題。
- 2026-09-18:使用者反應辭典查詢頁面(`find.html`)的搜尋方式可能要註記一下
  怎麼用。原本只有一行很簡短的提示文字,補了一個預設收合的「怎麼查?」
  `<details>` 說明區塊,四種輸入方式各配一個真的查得到的範例(中文意思
  「感謝」、台語漢字「食飯」、白話字/台羅「to-siā」/「kám-siā」、數字調
  「to1-sia7」、沒打調號的「to-sia」),每個範例都用 Playwright 實際查詢過
  一次確認真的有結果,不是憑印象編的。用原生 `<details>/<summary>` 而不是
  自己寫 JS 切換顯示,零 JS、有基本的無障礙語意。
- 2026-09-18:使用者反應手機上「在使用者還沒輸入前就有一個綠色框框」,附了
  一人刷題畫面的截圖,某個還沒作答的選項卻有綠色外框。原因:`.choiceBtn`
  跟 `.buzzBtn` 這些按鈕每一輪都是重複使用同一批 DOM element(只換文字
  內容跟 disabled 狀態,不是每題重新 create),但沒有針對這兩個 class
  自己定義 `:focus` 樣式,瀏覽器原生的 focus 樣式會殘留在「上一輪點過的
  那個按鈕」上——即使那個 DOM element 現在裝的是新一題的內容。瀏覽器
  預設 focus 樣式的顏色還可能被系統佈景主題影響(例如 Android Material
  You 動態配色),不一定是我們自己配色裡的綠色,只是恰好長得很像。修法是
  幫 `.choiceBtn`/`.buzzBtn` 都補上明確的 `:focus { outline: none }` +
  `:focus-visible { outline: ... }`——觸控點擊不算 focus-visible(瀏覽器
  原生行為),所以觸控不會留下對焦框;鍵盤 Tab 導覽還是會顯示,無障礙功能
  沒有被拿掉。用 Playwright 的裝置模擬(Pixel 7)搭配真正的 `tap()`(不是
  `click()`)重現過這個問題並確認修好,另外也測過鍵盤 Tab 導覽的 focus
  樣式還在。
...
- 2026-09-18:使用者反應「現在這個畫面太單調了」,整個 app 視覺重新設計成
  ...
  沒有動任何 JS 邏輯:
  - 設定畫面的每個 `<fieldset>` 都變成有陰影跟圓角的卡片;checkbox/radio
    從瀏覽器預設樣式改成藥丸狀「chip」——原生 input 用
    `clip: rect(0,0,0,0)` 視覺隱藏但維持 1×1px(不是 0×0,才不會在部分
    瀏覽器/報讀器的 tab 順序裡被跳過),用 CSS `:has(input:checked)`
    偵測選取狀態上色,不需要另外寫 JS 切換 class。「遊戲模式」「羅馬字
    系統」這兩組只有兩個選項的改用左右對切的 segmented control。
  - 新增一組 design token(`--radius-*`/`--shadow-*`/`--primary`/
    `--on-primary`),題目卡、選項按鈕、隊伍面板都套用陰影與圓角統一
    視覺語言;隊伍面板從整片色底改成白/深底 + 上方一條隊伍色細邊,更
    收斂。
  - 過程中兩個真的問題,都是靠 Playwright 實際截圖/量測試出來,不是肉眼
    看程式碼發現的:(1)`#teamOnlyFields` 新增的 `display:flex` 卡片樣式
    重踩了之前 `#soloStatsBox` 犯過的同一個 CSS specificity 坑(ID 選擇器
    蓋掉瀏覽器內建的 `[hidden]`),導致雙人模式限定欄位在一人模式下沒有
    真的隱藏,截圖比對才發現,補上 `#teamOnlyFields[hidden]{display:none}`
    修掉;(2)深色模式原本選了亮薄荷綠當 `--primary`,拿來當按鈕底色配
    白色文字,實際用程式算對比度只有 1.92:1(WCAG AA 一般文字門檻是
    4.5:1),幾乎看不清楚——這個問題光看截圖縮圖不會馬上發現,是寫一段
    小工具程式算 WCAG 相對亮度公式才抓到的。修法是拆出 `--on-primary`
    token,深色模式底色亮、文字改用接近黑色,淺色模式則是把 `--primary`
    從原本選的 `#059669`(配白字只有 3.77:1)換成更深的 `#047857`
    (配白字 5.48:1)。
